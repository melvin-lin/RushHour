...
void LCD_PIN_Init(void){
	...
	RCC->AHB2ENR |= RCC_AHB2ENR_GPIOBEN; // Port B - M
	RCC->AHB2ENR |= RCC_AHB2ENR_GPIOCEN; // Port C - M
	RCC->AHB2ENR |= RCC_AHB2ENR_GPIODEN; // Port D - M
...
void LCD_Configure(void){
	...
	// Configure DUTY[2:0] bits of LCD_CR and set the duty to 1/4 (011) - M
	LCD->CR &= ~(LCD_CR_DUTY);
	LCD->CR |= (LCD_CR_DUTY_1 | LCD_CR_DUTY_0);
	
	// Configure CC[2:0] bits of LCD_FCR and set the contrast value to maximum (111) - M
	LCD->FCR &= ~(LCD_FCR_CC);
	LCD->FCR |= (LCD_FCR_CC_2 | LCD_FCR_CC_1 | LCD_FCR_CC_0);
	
	// Configure PON[2:0] bits of LCD_FCR and set the pulse on period to maximum (111) - M
	LCD->FCR &= ~(LCD_FCR_PON);
	LCD->FCR |= (LCD_FCR_PON_2 | LCD_FCR_PON_1 | LCD_FCR_PON_0);
	
	// Disable the MUX_SEG segment of the LCD_CR - M
	LCD->CR &= ~(LCD_CR_MUX_SEG);

	// Select internal voltage as LCD voltage source - M
	LCD->CR &= ~(LCD_CR_VSEL);
	
	// Wait until the FCRSF flag of the LCD_SR is set (1) - M
	while ((LCD->SR & LCD_SR_FCRSF) == 0);
		
	// Enable the LCD by setting LCDEN bit of LCD_CR - M
	LCD->CR &= ~(LCD_CR_LCDEN);
	LCD->CR |= LCD_CR_LCDEN;
	
	// Wait until the LCD is enabled by checking the ENS bit of LCD_SR - M
	while ((LCD->SR & LCD_SR_ENS) == 0); 
	
	// Wait until the LCD booster is ready by checking the RDY bit of LCD_SR - M
